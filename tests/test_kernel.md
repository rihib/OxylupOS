...
```bash
% make DEBUG=1
clang -ffreestanding -fno-common -fno-pie -fno-stack-protector -I. -MD -mcmodel=medany -mno-relax -nostdlib -O2 -std=c2x -Wall -Werror -Wextra --target=riscv64 -ggdb3 -gdwarf-5 -DNPROC=1 -Wl,-Tkernel/kernel.ld -z max-page-size=4096 -Wl,-Map=kernel/kernel.map -o kernel/kernel.elf common/stdlib.c kernel/main.c kernel/proc.c kernel/riscvregs.c kernel/stack.c kernel/stackinit.S kernel/stdio.c kernel/trap.c kernel/uart.c
